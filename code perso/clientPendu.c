...
int main(int argc, char **argv) {
  ...
  void demandeJoueurs(char buffer2[]){
    ...
    log("**CLIENT joueurs recus:", buffer2);
    ligne_init++;
    char * joueur_i;
    char * joueur_points;
    char bufJoueurs[256];
    ...
    char copie[256];
    strcpy(copie, bufJoueurs);
    strcpy(copie,strchr(buffer2, ':')+1); // pe +1
    strcpy(bufJoueurs, copie);
  ...
  void demandeLettres(char buff_lettres[]){
	...
	log("**CLIENT bug_lettres:", buff_lettres);
    ligne_init++;
    ...
    char copie[200];
    strcpy(copie, buff_lettres);
    strcpy(copie,strchr(buff_lettres, ':')+1);
    strcpy(buff_lettres, copie);
	...
	strcpy(copie,lettresFausses);
    strcpy(copie,strchr(lettresFausses, ':')+1);
    strcpy(lettresFausses, copie);
  ...
  void finDuJeu(){
    ...
    if(scannedChar == 'Y'){
      ...
      log("**CLIENT envoi findeJeu:",envoi);
      //*5
      bzero(envoi,300);
      ...
      */
      ...
      //*6
      log("**CLIENT recu findeJeu:",envoi);
      char copie[200];
    ...
    mvwprintw(winInfos,1,2,"Nouveau Jeu !");
    wrefresh(winInfos);
  ...
  void *threadOthers(){
    ...
    while(/*bool_mot_incomplet==1 && lives > 0*/1){
      ...
      char typeMsg = recu[0];
      if (recu[1]!=':'){
        log("**CLIENT erreur de reception:",recu);
        break;
      }
      char copie[50];
  ...
  while(1){
      ...
      //nodelay(winHangman, 0);
      char logchar[2];
      logchar[0]=scannedChar;
      log("**CLIENT scannedChar :",logchar);
    if(finished==0){
